# -*- coding: utf-8 -*-
"""
Created on Tue Jun 10 12:36:59 2025
...
def objetive(trial):
    ...
    num_filters = trial.suggest_int('num_filters', 32, 128)  # Filtros en la capa Conv1D
    num_units = trial.suggest_int('num_units', 50, 300)  # numero de neuronas del modelo
    ...
    time_step = trial.suggest_int("time_step", 6,15)
    X_op, y_op = preparar_datos(X,lat_lon, 0, time_step)
    
    #construimos el modelo
    red = Sequential()
    red.add(Conv1D(filters = num_filters, activation = activation, kernel_size = 3, input_shape = (time_step,X_op.shape[2])))
    red.add(MaxPooling1D(pool_size = 2))
    red.add(Flatten()) #convierete todas las salidas del maxpooling en un solo vector de salida
    red.add(Dense(num_units, activation = activation))
    #Capa de salida
    ...
    metrica = perdida.history['val_loss'][-1]
    
    #ecm = sum((red.predict(X_op) - y_op)**2)/len(y_op)
...
plt.savefig(ruta_fig + "optimization_history_cnn.pdf",  bbox_inches='tight')
...
plt.savefig(ruta_fig + "hip_param_impot_cnn.pdf",  bbox_inches='tight')
...
plt.savefig(ruta_fig + "parallel_coordinate_cnn.pdf", bbox_inches='tight')
plt.rcdefaults()
...
with open(ruta_fig + 'df_opti_cnn.tex', 'w') as f:
