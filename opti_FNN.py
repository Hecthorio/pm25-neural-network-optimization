# -*- coding: utf-8 -*-
"""
Created on Thu Jun 12 13:09:04 2025
...
def objetive(trial):
    ...
    for i in range(num_layers):
        if i == 0:
            red.add(Dense(num_neurons, input_dim = X_op.shape[1], activation=activation))
        else:
...
plt.savefig(ruta_fig + "optimization_history_fnn.pdf",  bbox_inches='tight')
...
plt.savefig(ruta_fig + "hip_param_impot_fnn.pdf",  bbox_inches='tight')
...
plt.savefig(ruta_fig + "parallel_coordinate_fnn.pdf", bbox_inches='tight')
plt.rcdefaults()
...
with open(ruta_fig + 'df_opti_fnn.tex', 'w') as f:
